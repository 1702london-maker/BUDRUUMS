...
export default function AdminReferralsPage() {
  ...
  async function load(adminKey: string) {
    ...
    if (res.status === 401) { setMsg("Wrong key."); setLoading(false); return; }
      ...
      <div style={{ maxWidth: 800, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 32 }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: "#6B6B6B", marginTop: 4 }}>{pending.length} pending · {apps.length} total</p>
          </div>
          <button onClick={() => load(key)} style={{ padding: "9px 18px", border: "1px solid #E8E8E8", borderRadius: 4, background: "#fff", fontSize: 13, cursor: "pointer", color: "#6B6B6B" }}>
            Refresh
          </button>
        </div>

        {msg && (
          <div style={{ padding: "14px 18px", background: msg.startsWith("Error") ? "#fef2f2" : "#f0fdf4", border: `1px solid ${msg.startsWith("Error") ? "#fecaca" : "#bbf7d0"}`, borderRadius: 6, marginBottom: 20, fontSize: 14, color: msg.startsWith("Error") ? "#ef4444" : "#22c55e" }}>
            {msg}
          </div>
        )}
                ...
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                      ...
                      <span style={{ fontSize: 11, fontWeight: 600, padding: "3px 10px", borderRadius: 20, background: statusColor[app.status] + "22", color: statusColor[app.status], textTransform: "capitalize" }}>
                        {app.status}
                      </span>
                    </div>
