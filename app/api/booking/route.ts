...
<title>New Booking — Budruum</title></head>
...
function confirmHtml(name: string, service: string) {
  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/>
<title>Booking Confirmed — Budruum</title></head>
<body style="margin:0;padding:0;background:#F2F2F2;font-family:'DM Sans',Helvetica,Arial,sans-serif;">
<table width="100%" cellpadding="0" cellspacing="0" style="background:#F2F2F2;padding:40px 0;">
<tr><td align="center">
<table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,.08);">
  ...
  <tr><td style="padding:40px;">
    ...
    <div style="margin:0 0 32px;padding:24px;background:${LIGHT};border-radius:6px;border:1px solid #E8E8E8;">
      ...
      ${["Our team reviews your brief and confirms a suitable time slot","You receive a calendar invite and a short preparation guide","Your 30-minute session runs — focused, structured, actionable"].map(s => `<p style="margin:10px 0 0;font-size:14px;color:${GREY};line-height:1.7;padding-left:16px;border-left:2px solid ${TERRACOTTA};">${s}</p>`).join("")}
    </div>
    <table cellpadding="0" cellspacing="0"><tr><td style="border-radius:4px;background:${TERRACOTTA};">
      <a href="https://budruum.co.uk/insights" style="display:inline-block;padding:13px 28px;font-size:13.5px;font-weight:500;color:#ffffff;text-decoration:none;letter-spacing:.03em;">Read Our Insights →</a>
    </td></tr></table>
  </td></tr>
...
export async function POST(req: NextRequest) {
  ...
  try {
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "Budruum Bookings <info@budruum.co.uk>",
    ...
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [email],
        subject: "Your consultation request is confirmed — Budruum",
