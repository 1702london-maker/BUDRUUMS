...
<table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,.08);">
  ...
  <tr><td style="padding:44px 44px 32px;">
    ...
    <p style="margin:0 0 18px;font-size:15px;color:${GREY};line-height:1.85;">Thank you for subscribing to the Budruum newsletter. You will receive practical insights, business strategy, and founder resources — delivered straight to your inbox, without the noise.</p>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="background:${LIGHT};border:1px solid #E8E8E8;border-radius:6px;margin-bottom:32px;">
      <tr><td style="padding:24px 28px 8px;">
        ...
        <a href="https://budruum.co.uk${href}" style="display:block;padding:14px 0;border-top:1px solid #E8E8E8;text-decoration:none;">
          <p style="margin:0;font-size:14px;color:${DARK};line-height:1.5;">${title} <span style="color:${TERRACOTTA};">→</span></p>
        </a>`).join("")}
      </td></tr>
    </table>
  ...
  <tr><td style="padding:14px 44px;text-align:center;">
    <p style="margin:0;font-size:11px;color:#C0C0C0;">You subscribed at budruum.co.uk · <a href="https://budruum.co.uk" style="color:#C0C0C0;text-decoration:none;">Unsubscribe</a></p>
  </td></tr>
</table>
</td></tr></table>
</body></html>`;
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    await fetch("https://api.resend.com/emails", {
      ...
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${key}` },
      body: JSON.stringify({
        from: "Budruum Insights <info@budruum.co.uk>",
        to: [email],
